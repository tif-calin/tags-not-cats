...
export const feedbinServiceHooks: ServiceHooks = {
  ...
  fetchItems: () => async (_, getState) => {
    ...
    do {
      try {
        ...
        items.push(
          ...lastFetched.filter(i => i.id > configs.lastId && i.id < min)
        )
    ...
    if (items.length > 0) {
      ...
      items.forEach(i => {
        ...
        if (starred.has(i.id) !== Boolean(item.starred))
          markItems(configs, "starred", item.starred ? "POST" : "DELETE", [
            i.id,
          ])
  ...
  markRead: (item: RSSItem) => async (_, getState) => {
    await markItems(getState().service as FeedbinConfigs, "unread", "DELETE", [
      parseInt(item.serviceRef),
    ])
  },

  markUnread: (item: RSSItem) => async (_, getState) => {
    await markItems(getState().service as FeedbinConfigs, "unread", "POST", [
      parseInt(item.serviceRef),
    ])
  },

  star: (item: RSSItem) => async (_, getState) => {
    await markItems(getState().service as FeedbinConfigs, "starred", "POST", [
      parseInt(item.serviceRef),
    ])
  },

  unstar: (item: RSSItem) => async (_, getState) => {
    await markItems(getState().service as FeedbinConfigs, "starred", "DELETE", [
      parseInt(item.serviceRef),
    ])
  },
}
