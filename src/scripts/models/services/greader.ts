    ...
    if (items.length > 0) {
      ...
      items.map(i => {
        ...
        if (source.rules) {
          ...
          if (item.hasRead !== hasRead) editTag(configs, item.serviceRef, READ_TAG, item.hasRead);
          if (item.starred !== starred) editTag(configs, item.serviceRef, STAR_TAG, item.starred);
        }
