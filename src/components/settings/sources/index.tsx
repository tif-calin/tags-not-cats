...
class SourcesTab extends React.Component<SourcesTabProps, SourcesTabState> {
  ...
  private _onColumnClick = (_ev: React.MouseEvent<HTMLElement>, selectedColumn: IColumn) => {
    const currSortBy = this.state.sortBy;
    const currSortDescending = this.state.sortDescending;
    const selectedSortBy = selectedColumn.fieldName;

    if (currSortBy === selectedSortBy) {
      if (currSortDescending) this.setState({ sortDescending: false });
      else this.setState({ sortBy: undefined, sortDescending: true });
    } else this.setState({ sortBy: selectedSortBy, sortDescending: true });
  };
}
